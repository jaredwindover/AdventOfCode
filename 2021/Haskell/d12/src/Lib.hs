...
update cs ((c, path, visited):rest, paths) = (newOptions ++ rest, newPaths)
    where
      newOptions = map (\c' -> (c', path ++ [c'], newVisited)) realPotentials :: [SoFar]
      newVisited
      ...
      realPotentials = filter (\s -> (s/="end") && ((not $ isSmall s) || (not $ member s visited))) potentials
